    ...
    httpAsync(null,"/source/get/EQ?group="+group+"&sub_group="+sub_group,"GET",function(result){
      ...
      for(let i =0; i<data.length; i++){
        ...
        if(SHOW_LIST_METER_TYPE == 1){
          ...
          httpAsync(null,"/source/get/draw_map_zone_map?group="+group+"&sub_group="+sub_group,"GET",function(result){
            let draw_map_zone_map = JSON.parse(result);
            load_map_meter_list(data,draw_map_zone_map);
          })
        }else{
...
function load_map_meter_list(data,draw_map_zone_map){
  // console.log(data)
  $("#totalSource").html(data.length);
  let uluru = {
      ...
      // if(data[i].branch_total == 1){
        marker_2 = new google.maps.Marker({
          ...
          icon: {
            ...
            scaledSize: new google.maps.Size(18, 28), // scaled size
  ...
  for(let i=0; i<draw_map_zone_map.length; i++){
    if(draw_map_zone_map[i].draw_map != null){
      let points = JSON.parse(draw_map_zone_map[i].draw_map);
      for(let j=0; j<points.length; j++){
          let flightPath = new google.maps.Polyline({
          path: [points[j].point_1,points[j].point_2],
          geodesic: true,
          strokeColor: "#FF0000",
          strokeOpacity: 1.0,
          strokeWeight: 3,
        });
        flightPath.setMap(map_2);
      }
    }
    if(draw_map_zone_map[i].zone_map != null){
      let zone_paths = JSON.parse(draw_map_zone_map[i].zone_map);
