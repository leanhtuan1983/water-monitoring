...
function initialize(passport){
    var authenticateUser = (req, username, password, done) => {
        ...
        let sqlquery = " SELECT * FROM account WHERE usr = ? AND status = ?"
    ...
    var registerAuthenticate = (req,username,password,done) =>{
        ...
        if(username == ""){
            ...
            accessDB("SELECT * FROM account WHERE usr = ? OR email = ?",[username,req.body.email],function(result){
                if(result.length > 0){
                    if(result[0].usr == username){
                    ...
                    accessDB("INSERT INTO account (usr, email, pass, role, status) VALUES (?,?,?,?,?)",[username,req.body.email,x,"sub",1],function(result){
                        console.log(result);
                        accessDB("SELECT * FROM account WHERE usr = ? AND status = ?",[username,1],function(result2){
                            if(result2.length > 0){
    ...
    passport.deserializeUser((id, done) =>{
        accessDB(" SELECT * FROM account WHERE id = ?",[id], function( result){
            done(null, result[0]);
        });
    })
}
